...
import com.lyle.common.sequence.DataSourceWeightRandom;
...
public class MultipleSequenceDAO {
	...
	/** 随机权重 */
	private DataSourceWeightRandom weightRandom;
	/** sequence表名默认值 */
	...
	public void init() {
		...
		weightRandom = new DataSourceWeightRandom(dataSourceNum);
		...
		for (SequenceDataSourceHolder dsHolder : dataSourceList) {
			dsHolder.setParameters(getTableName(), getSelectSql(), getUpdateSql(), getInsertSql(), adjust);
		}
		isInitialize = true;
	}
	...
	public void initSequenceRecord(String sequenceName, long minValue, long maxValue, int innerStep)
		...
		for (int index = 0; index < dataSourceNum; index++) {
			...
			dsHolder.initSequenceRecord(index, sequenceName, innerStep, innerStep * dataSourceNum, minValue, maxValue,
					getValueColumnName());
		}
	}
	...
	public Map<String, Map<String, Object>> getAllSequenceNameRecord() throws SQLException {
		...
		for (int i = 0; i < dataSourceList.size(); i++) {
			try {
				...
				sequenceRecordMap = dsHolder.getAllSequenceRecordName(getSelectAllRecord(), getNameColumnName(),
	...
	public Map<String, Map<String, Object>> getSequenceRecordByName(String sequenceName)
		...
		sequenceRecordMap = dsHolder.getSequenceRecordByName(getSequenceRecordSql(), getMinValueColumnName(),
				getMaxValueColumnName(), getInnerStepColumnName(), sequenceName);
		return sequenceRecordMap;
	}
	...
	public SequenceRange nextRange(String sequenceName, long minValue, long maxValue, int innerStep)
		...
		for (int i = 0; i < retryTimes; i++) {
			...
			for (int j = 0; j < dataSourceNum; j++) {
				...
				sequenceRange = dsHolder.tryOnSelectedDataSource(index, sequenceName, minValue, maxValue, innerStep,
						innerStep * dataSourceNum, excludeIndexes);
		...
		throw new SequenceException("MultipleSequenceDao没有可用的数据源了,数据源个数dataSourceNum=" + this.dataSourceNum
				+ ",重试次数retryTimes=" + this.retryTimes);
	}
